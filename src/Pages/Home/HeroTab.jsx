...
export const HeroTab = () => {
  ...
  return (
    <div className='min-h-[calc(100vh-100px)] w-full  grid grid-cols-2 place-items-center '>
        {card.map((item)=>(
            <div className='h-[250px] w-[450px] relative' key={item.id}><div className='w-full h-[90%]  rounded-md overflow-hidden ' onClick={()=> navigate(`/hero/${item.id}`)} ><img src={item.img} className='rounded-md h-full w-full hover:scale-110 transition-all hover:sk' alt="" /></div>
